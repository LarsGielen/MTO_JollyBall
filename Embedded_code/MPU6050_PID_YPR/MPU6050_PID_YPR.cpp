...
void writeMPU6050Register(uint8_t reg, uint8_t val) {
...
void loop() {
    ...
    // otherwise, check for DMP data ready interrupt (this should happen frequently)
        ...
        if (!calibrated) {
            ...
            float calibratedYaw = ypr[0] * 180/M_PI - yprOffset[0];
            float calibratedPitch = ypr[1] * 180/M_PI - yprOffset[1];
            float calibratedRoll = ypr[2] * 180/M_PI - yprOffset[2];
            
            // Apply additional noise filtering with moving average
            ...
            filteredYaw = filteredYaw * (1 - alpha) + calibratedYaw * alpha;
            filteredPitch = filteredPitch * (1 - alpha) + calibratedPitch * alpha;
            filteredRoll = filteredRoll * (1 - alpha) + calibratedRoll * alpha;
...
void calibrateYPR() {
    ...
    if (sampleCount < calibrationSamples) {
        // Accumulate samples
        yprSum[0] += ypr[0] * 180/M_PI;
        yprSum[1] += ypr[1] * 180/M_PI;
        yprSum[2] += ypr[2] * 180/M_PI;
